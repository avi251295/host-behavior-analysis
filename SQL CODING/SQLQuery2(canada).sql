...
select top 10 * from host_toronto_df
select top 10 * from review_toronto_df
select top 10 * from listing_toronto_df

update host_toronto_df
set host_acceptance_rate=0, host_is_superhost=0,host_has_profile_pic='TRUE'
where host_acceptance_rate is null or host_is_superhost is null or host_has_profile_pic is null ;
...
FROM host_toronto_df h
JOIN listing_toronto_df l ON h.host_id = l.host_id
GROUP BY host_is_superhost;

--vancouver


update host_vancouver_df
set host_response_rate=0, host_acceptance_rate=0, host_is_superhost=0,
host_listings_count=0, host_has_profile_pic=0, host_identity_verified=0
where host_response_rate is null or host_acceptance_rate is null or
host_is_superhost is null or host_listings_count is null or host_has_profile_pic is null or host_identity_verified is null

alter table host_vancouver_df
alter column host_has_profile_pic int
alter table host_vancouver_df
alter column host_identity_verified int
alter table listing_vancouver_df
alter column instant_bookable int
...
avg(accommodates) as Avg_Accomodates, max(bedrooms) as Max_Bedrooms,
min(price) as Min_Price
FROM host_vancouver_df h
JOIN listing_vancouver_df l ON h.host_id = l.host_id
GROUP BY host_is_superhost;
...
FROM host_toronto_df h
JOIN listing_toronto_df l ON h.host_id = l.host_id
JOIN review_toronto_df r ON l.id = r.listing_id
...
FROM host_vancouver_df h
JOIN listing_vancouver_df l ON h.host_id = l.host_id
JOIN review_vancouver_df r ON l.id = r.listing_id
where r.comments like '%like%' or r.comments like '%good%' or r.comments like '%love%'  
group by h.host_is_superhost 
